  ...
  return (
    <div className="relative group border border-cyan-700 p-6 cursor-pointer">
     ...
     <div className="flex items-start justify-between mt-4 space-x-4">
        <div className="font-bold text-gray-900 sm:text-sm text-x5 md:text-base">
            <p className="w-{100px} overflow-hidden text-ellipsis whitespace-noormal">{singleProductTile?.title}</p>
        </div>
     ...
     <button disabled={cartItems.findIndex(item=> item.id === singleProductTile.id)>-1} onClick={()=>handleAddToCart(singleProductTile)} className=" disabled:opacity=65 px-5 mt-5 w-full py-2 rounded-none bg-black text-white font-bold text-lg">Add To Cart </button>
    </div>
  )
}
